...
def main(cfg: DictConfig):
    ...
    dataset_dir = (
        logdir / "dataset"
        if cfg["write_to_dataset"] is None
        else Path(cfg["write_to_dataset"])
    )
    ...
    for i in range(start, end):
        ...
        for eid, (em, eval) in enumerate(zip(scheduler.emitters, evaluators)):
            ...
            all_task_id += [em.task_id] * cfg["qd"]["emitter"]["batch_size"]
            ...
            for entropy, env_rollouts in zip(objective, trajectories):
                if entropy > 0.01:
                    for traj in env_rollouts:
                        if traj.success:
                            temp_dataset.write_episode(trajectory=traj)
    ...
    # dataset
    temp_dataset.convert_to_lerobot(cfg["lerobot_dataset_repo_id"])
