...
from libero_spatial_eval import Trajectory
...
def _add_trajectory_to_dataset(trajectory: Trajectory, dataset: LeRobotDataset) -> None:
    for image, wrist_image, state, action in zip(
        trajectory.image, trajectory.wrist_image, trajectory.state, trajectory.action
    ):
        dataset.add_frame(
            {
                "image": image,
...
def generate_all_train_assets(
    scheduler_path: str,
    repo_id: str = "shihanzh/qdpref",  # FIXME
) -> None:
    with open(file=scheduler_path, mode="rb") as archive_file:
        ...
        for cell in tqdm(archive):
            ...
            for traj in cell["trajectories"]:
                with _suppress_tqdm():
                    _add_trajectory_to_dataset(traj, dataset)
