...
@dataclass
class Trajectory:
    prompt: Optional[str] = None  # TODO: Find some way to merge this with env params
    success: bool = False
...
class LiberoSpatialEval:
        ...
        **kwargs,
        ...
        if self._dask_client is not None:
            ...
            if self.num_trials_per_sol > nworkers:
                logger.warning(f"num_trials_per_sol={self.num_trials_per_sol} exceeds the number of workers {nworkers}")
    ...
    def evaluate_single(self, solution: np.ndarray) -> Tuple[np.ndarray, float, Tuple[float, float], List[Trajectory]]:
        ...
        try:
            ...
            return (
                ...
                1e-6,
                (0.0, 0.0),
                [Trajectory(success=False) for _ in range(self.num_trials_per_sol)],
            )
    ...
    def evaluate(self, solutions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[List[Trajectory]]]:
        ...
        for sol_id, sol in enumerate(solutions):
            logger.info(f"------------------- Evaluating solution {sol_id + 1}/{batch_size} (task_id: {self.task_id})")

            rep, obj, meas, traj = self.evaluate_single(sol)

            trial_success = [f"trial{tid}: {'success' if tr.success else 'fail'}" for tid, tr in enumerate(traj)]
            logger.info(f"\n{trial_success}")
            logger.info(f"\n\t objective: {obj}\n\t measures: {meas}")
        ...
        return np.array(repaired), np.array(objective), np.array(measures), trajectories
...
def rollout(
    ...
    if video_logdir is not None:
        imageio.mimwrite(
            sol_logdir / f"trial{trial_id}_{'success' if trajectory.success else 'fail'}.mp4",
